...
if %errorlevel% neq 0 (echo [FAIL] Node.js not found >> %L% & echo ERROR: Node.js not found! & pause & exit /b 1)
...
if %errorlevel% equ 0 (echo [INFO] Port 3000 occupied, freeing... >> %L% & for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":3000"') do taskkill /PID %%P /F >nul 2>&1 & timeout /t 2 /nobreak >nul)
...
if not exist "%~dp0data\village.db" (
    ...
    %NODE% scripts\setup.js >> %L% 2>&1
    if %errorlevel% neq 0 (echo [FAIL] Init failed >> %L% & echo ERROR: Init failed! Check startup.log for details. & pause & exit /b 1)
