...
// GET /api/public-service/search-families?q=xxx — 模糊搜索农户姓名（公开接口）
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const families = await prisma.family.findMany({
      ...
      select: { headName: true, headPhone: true },
      take: 8,
    });
    ...
    const results = [
      ...families.map(f => ({ name: f.headName, phone: f.headPhone || "", label: f.headName })),
