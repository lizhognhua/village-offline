...
const TAG_OPTIONS = ["党员", "重病重疾", "高龄老人", "留守儿童", "残疾"];
...
function pointInPolygon(point, polygon) {
  ...
  const [ffName,setFfName] = useState(""); const [ffPop,setFfPop] = useState(1); const [ffAttr,setFfAttr] = useState("一般农户"); const [ffTags,setFfTags] = useState<string[]>([]);
  ...
  // Save family
  const saveFamily=async function(){if(!ffName.trim()||!clickLatLng.current)return;setSaving(true);try{var fd=new FormData();fd.append("headName",ffName.trim());fd.append("population",String(ffPop));fd.append("familyAttr",ffAttr);fd.append("tags",JSON.stringify(ffTags));fd.append("headPhone",ffPhone);fd.append("spouseName",ffSpouseName);fd.append("spousePhone",ffSpousePhone);fd.append("spouseIdCard",ffSpouseIdCard);fd.append("address",ffAddress);fd.append("income",ffIncome);fd.append("notes",ffNotes);fd.append("latitude",String(clickLatLng.current.lat));fd.append("longitude",String(clickLatLng.current.lng));if(editFamily){var body:any={headName:ffName.trim(),population:ffPop,familyAttr:ffAttr,tags:JSON.stringify(ffTags),headPhone:ffPhone,spouseName:ffSpouseName,spousePhone:ffSpousePhone,spouseIdCard:ffSpouseIdCard,address:ffAddress,income:ffIncome,notes:ffNotes,photos:JSON.stringify(ffPhotos),latitude:clickLatLng.current.lat,longitude:clickLatLng.current.lng};await fetch("/api/village/families/"+editFamily.id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});}else{fd.append("photoUrls",JSON.stringify(ffPhotos));await fetch("/api/village/families",{method:"POST",body:fd});}setFamilyFormOpen(false);setEditFamily(null);loadData();}catch{alert("保存失败");}finally{setSaving(false);}};
  ...
  const startNewFamily=function(){setFamilyPickerOpen(false);setEditFamily(null);setFfName("");setFfPop(1);setFfAttr("一般农户");setFfTags([]);setFfPhone("");setFfSpouseName("");setFfSpousePhone("");setFfSpouseIdCard("");setFfAddress("");setFfIncome("");setFfPhotos([]);setFfNotes("");setFamilyFormOpen(true);};
